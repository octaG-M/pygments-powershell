...
"""
    ...
    Nov. 25, 2018: several modifications have been performed to the
    PowerShellLexer (class) code. They are documented inside the code.
"""
...
class PowerShellLexer(RegexLexer):
    ...
    Nov. 25, 2018: several modifications were performed to
    the code below:
    
      * to deal correctly with both the ``ForEach-Object`` entity and its
        alias ( ``foreach`` ), the word **foreach** has been removed from
        the lists provided by ``keywords`` and ``verbs``
      * the list ``ambiguous`` and two regexes have been introduced
      * the issue of correctly lexing the entities ``env:``, ``C:\`` and
        others has been addressed
      * the issue of correctly lexing the entity ``function:Clear-Host``
        has been addressed, using one of the tricks introduced by Mr. PrsPrsBK
        on GitHub, `sources <https://github.com/PrsPrsBK/pygments-powershell>`_.
    """
    name = 'PowerShell'
    ...
    # Nov. 25, 2018: removed 'foreach'
    keywords = (
        'while validateset validaterange validatepattern validatelength '
        'validatecount until trap switch return ref process param parameter in '
        'if global: function for finally filter end elseif else '                       
        'dynamicparam do default continue cmdletbinding break begin alias \\? '
        '% #script #private #local #global mandatory parametersetname position '
        'valuefrompipeline valuefrompipelinebypropertyname '
        'valuefromremainingarguments helpmessage try catch throw').split()
    
    # Nov. 25, 2018: removed 'file'
    operators = (
    ...
    # Nov. 25, 2018: removed 'foreach', added 'unblock'
    verbs = (
        ...
        'group get format export expand exit enter enable disconnect '                  
        'disable debug cxnew copy convertto convertfrom convert connect '
        'complete compare clear checkpoint aggregate add unblock').split()              
    ...
    tokens = {
        'root': [
            ...
            (r'(\$|@@|@)((global|script|private|env):)?\w+',
             Name.Variable),
            # Nov. 25, 2018: added to deal with 'ForEach-Object'
            (r'(%s)-[a-z_]\w*\b' % '|'.join(ambiguous), Name.Builtin),
            # Nov. 25, 2018: added to deal with the alias 'foreach'
            (r'(%s)\b' % '|'.join(ambiguous), Name.Builtin),                           
            (r'(%s)\b' % '|'.join(keywords), Keyword),
            (r'-(%s)\b' % '|'.join(operators), Operator),
            (r'(%s)-[a-z_]\w*\b' % '|'.join(verbs), Name.Builtin),
            (r'\[[a-z_\[][\w. `,\[\]]*\]', Name.Constant),  # .net [type]s
            # Nov. 25, 2018: modified from 'Name' to 'Name.Builtin' to deal
            # with '-Force', '-Recurse' and others
            (r'-[a-z_]\w*', Name.Builtin),
            # Nov. 25, 2018: added to deal with 'C:\ ',  
            (r'(?:\w:\\\s?)', Name.Builtin),  
            # Nov. 25, 2018: added to deal with 'env: ',
            (r'(?:\w{2,}:\s?)', Name.Builtin),                                             
            (r'\w+', Name),
            (r'[.,;@{}\[\]$()=+*/\\&%!~?^`|<>-]|::', Punctuation),
            # Nov. 25, 2018: added to deal with 'function:Clear-Host'
            # An idea of Mr. PrsPrsBK:
            (r'[\w+-\.:\\]+', Keyword),                                                     
        ],
        'child': [
            (r'\)', Punctuation, '#pop'),
